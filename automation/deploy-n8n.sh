...
collect_domain() {
    ...
    while true; do
        ...
        if validate_domain "$base_domain"; then
            ...
            echo ""
            log_info "n8n will be accessible at: $DOMAIN"
            echo ""
...
verify_dns() {
    ...
    if [[ -z "$resolved_ip" ]]; then
        ...
        echo ""
        echo "Please create an A record for the n8n subdomain:"
        echo "  Type: A"
        echo "  Name: n8n"
        echo "  Value: $VPS_IP"
...
verify_deployment() {
    ...
    # Check Redis (with retry logic - Redis can take 10-30 seconds to fully start)
    ...
    if [[ "$http_response" == "301" ]] || [[ "$http_response" == "308" ]]; then
        log_success "HTTP to HTTPS redirect working (${http_response})"
...
configure_automated_backups() {
    ...
    # Test backup script (needs sudo to read .env file with chmod 600)
    log_info "Testing backup script..."
    if sudo bash "$INSTALL_DIR/scripts/backup-postgres.sh" >> "$LOG_FILE" 2>&1; then
...
check_automated_backups() {
    log_step "Checking Automated Backup Configuration"
    
    echo ""
    echo "Checking cron jobs for automated backups..."
    echo ""
    
    if crontab -l 2>/dev/null | grep -q "backup-all.sh"; then
        log_success "Automated backups are configured"
        echo ""
        echo "Current backup schedule:"
        crontab -l 2>/dev/null | grep "backup-all.sh"
        echo ""
        echo "Next scheduled backup:"
        # Show next cron execution time
        echo "  Daily at 2:00 AM (based on cron job)"
        echo ""
        
        # Check if backups directory exists and show last backup
        if [[ -d "$INSTALL_DIR/backups/postgres" ]]; then
            local last_backup=$(ls -t "$INSTALL_DIR/backups/postgres"/*.sql.gz 2>/dev/null | head -1)
            if [[ -n "$last_backup" ]]; then
                local backup_date=$(stat -c %y "$last_backup" 2>/dev/null || stat -f %Sm "$last_backup" 2>/dev/null)
                ...
                log_warning "No backups found yet (first backup will run at 2 AM)"
            fi
        fi
    else
        log_warning "Automated backups are NOT configured"
        echo ""
        ...
        echo "  crontab -e"
        echo "  0 2 * * * $INSTALL_DIR/scripts/backup-all.sh >> /var/log/n8n-backup.log 2>&1"
    fi
...
show_usage() {
    ...
    echo "  --check-backups           Check automated backup configuration"
    echo ""
    echo "EXAMPLES:"
    ...
    echo "  $0 --check-health"
    echo ""
    echo "  # Check if backups are configured"
    echo "  $0 --check-backups"
    echo ""
    exit 0
}
